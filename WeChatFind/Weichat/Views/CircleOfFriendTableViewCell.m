...
#import "CircleOfFriendTableViewCell.h"

@implementation CircleOfFriendGrayView
...
@implementation CircleOfFriendThumCommentView
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_cell.delegate != nil && [_cell.delegate respondsToSelector:@selector(DynamicsCell:didClickComment:)]) {
        [_cell.delegate DynamicsCell:_cell didClickComment:(CircleOfFriendCommentItemModel *)_cell.layout.model.commentArr[indexPath.row]];
    }
}
...
@implementation CircleOfFriendTableViewCell
...
-(void)setLayout:(CircleOfFriendLayout *)layout
{
    UIView * lastView;
    _layout = layout;
    ...
    if (model.likeArr.count != 0 || model.commentArr.count != 0) {
        ...
        [_thumbCommentView setWithLikeArr:model.likeArr CommentArr:model.commentArr CircleOfFriendLayout:layout];
    }else{
...
{
    CircleOfFriendModel * model = _layout.model;
    if (!model.isThumb) {//点赞
        if (!_jrMenuView) {
...
-(CircleOfFriendGrayView *)grayView
{
    if (!_grayView) {
...
-(CircleOfFriendThumCommentView *)thumbCommentView
{
